...
void btt_tempo_tracking              (BTT* self)
{
  ...
  */
  ...
  int* candidate_tempo_lags = calloc(self->num_tempo_candidates, sizeof(*candidate_tempo_lags));
  if(candidate_tempo_lags == NULL)
    return;
  ...
  float* score_max      = calloc(self->num_tempo_candidates, sizeof(*score_max));
  float* score_variance = calloc(self->num_tempo_candidates, sizeof(*score_variance));
  if((score_max == NULL) || (score_variance == NULL))
    {
      free(candidate_tempo_lags);
      free(score_max);
      free(score_variance);
      return;
    }
